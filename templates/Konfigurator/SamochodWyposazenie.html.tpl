...
<div class="container-fluid mt-5">
        ...
        {else}
            <div class="row m-5">
                <div class="col-8">
                    ...
                    <form class="needs-validation2" novalidate action="http://{$smarty.server.HTTP_HOST}{$subdir}Lakier"
                          method="post">
                        ...
                        {foreach $wyposazenie as $key => $Wartosc}
                            ...
                            {if {$Wartosc['id_Opcja']} == 1}
                                <div class="custom-control custom-radio custom-control-inline">
                                    <input {if isset($smarty.session.id_SamochodWyposazenie)} {if $Wartosc['id_SamochodWyposazenie'] === $smarty.session.id_SamochodWyposazenie}} checked="checked" {/if} {/if}
                                            class="custom-control-input" type="radio" name="id_SamochodWyposazenie"
                                            id="inlineRadio{$Wartosc['id_SamochodWyposazenie']}"
                                            value="{$Wartosc['id_SamochodWyposazenie']}" required>

                                    <label class="custom-control-label"
                                           for="inlineRadio{$Wartosc['id_SamochodWyposazenie']}">
                                        <div style="max-width: 500px" class="text-center">
                                            ...
                                            {if (isset($prawo) && ($prawo == 'admin'))}
                                                <div class="text-center">
                                                    <a class="btn btn-danger" href="http://{$smarty.server.HTTP_HOST}{$subdir}Tapicerka/delete/{$Wartosc['id_SamochodWyposazenie']}">Usuń</a>
                                                </div>
                                            {/if}
                                        </div>
                                    </label>
    ...
    {if (isset($prawo) && ($prawo == 'admin'))}
        <div class="container">
            <form id="add_silnik" action="http://{$smarty.server.HTTP_HOST}{$subdir}Tapicerka/add-2" method="post">

                <input type="hidden" name="id_zbior_modeli" value="{$smarty.session.id_ZbiorModeli}}">

                <div class="form-group">
                    <label for="name">Tapicerka</label>
                    {html_options name=id_SamochodWyposazenie options=$selectwyposazenie  class="form-control"}
                </div>
                ...
                <button type="submit" name="submit" class="btn btn-secondary">Dodaj tapicerke do modelu</button>
            </form>
